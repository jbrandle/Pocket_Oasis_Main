...
const Hub: React.FC<HubProps> = ({ 
  user, onEnterZone, onOpenJob, onOpenShop, onOpenArcade, onOpenSocial, onOpenSettings, onOpenCalendar, onLogout, onClearNotifications, onReadNotification, onSetXPGoal, onToggleTimeFormat, onPurchasePass
}) => {
      ...
      <header className="shrink-0 z-40 bg-white/5 backdrop-blur-3xl border-b border-white/10 px-4 py-4 grid grid-cols-[auto_1fr_auto] items-center gap-4 relative shadow-2xl">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <h2 className="font-black text-[11px] tracking-tight leading-none uppercase italic truncate max-w-[70px] mb-1">{user.avatar.name}</h2>
            <div className="flex flex-col gap-0.5">
              <span className="text-[8px] font-black text-oasis-cyan/60 uppercase tracking-tighter leading-none">LVL {user.level}</span>
              <span className="text-[8px] font-black text-oasis-cyan/60 uppercase tracking-tighter leading-none">XP {todayXP}</span>
            </div>
          </div>
        </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3 px-1">
               <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.6em] flex items-center gap-2">
                 <Sparkles size={12} className="text-oasis-cyan" /> Oasis Horizons
               </h3>
                 ...
                 {selectedActivityDate.toDateString() === new Date().toDateString() ? (
                   <>
                      ...
                      <div className="space-y-4">
                         ...
                         <OpportunityItem 
                            icon={<Brain size={18}/>} 
                            title="Void Library Scan" 
                            reward="100 XP" 
                            color="text-indigo-400" 
                            onClick={() => { setShowActivities(false); handleSectorClick(ZoneType.LIBRARY); }} 
                         />
                         <OpportunityItem 
                            icon={<Battery size={18}/>} 
                            title="Hardware Sync" 
                            reward="Passive Yield Boost" 
                      ...
                      <div className="pt-4 text-center">
                        <p className="text-[7px] text-gray-600 font-black uppercase tracking-[0.2em]">End of Live Data Stream</p>
                      </div>
                   </>
                 ) : (
                   <div className="h-64 flex flex-col items-center justify-center text-gray-700 space-y-4 grayscale opacity-40">
                      <div className="p-6 rounded-full bg-white/5 border border-white/5 shadow-inner">
                        <Clock size={40} />
                      </div>
                      <p className="text-[10px] font-black uppercase tracking-[0.5em] text-center px-4">Temporal Log Sealed for this Cycle</p>
                   </div>
                 )}
              </div>
